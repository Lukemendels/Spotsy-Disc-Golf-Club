...
export const AdminPage: React.FC<AdminPageProps> = ({
  ...
  const [activeSubTab, setActiveSubTab] = useState<"events" | "rounds" | "beginnerSignups" | "subs" | "seed">("events");
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-6 rounded-2xl text-white shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-xl">
            Publish official events, moderate casual player posts, review beginner clinic signups, and manage announcement email lists.
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-1.5 bg-white p-1.5 rounded-xl border border-slate-200 card-shadow text-xs font-bold">
        ...
        >
          Moderation / Rounds ({rounds.length})
        </button>

        <button
          onClick={() => setActiveSubTab("beginnerSignups")}
          className={`px-3 py-1.5 rounded-lg transition ${
                ...
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-slate-500">
                      {new Date(evt.startDateTime).toLocaleString()}
                    </span>
                  </div>
              ...
              >
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-slate-500">
                      Org: {rnd.organizerName}
                    </span>
                  </div>
                  <h4 className="font-bold text-slate-900 text-sm">Course ID: {rnd.courseId} ({rnd.layout})</h4>
                  <p className="text-xs text-slate-500">
                    Tee: {new Date(rnd.teeTime).toLocaleString()} | Players: {rnd.participantIds?.length}/{rnd.maxCapacity}
                  </p>
                </div>
        ...
        <div className="space-y-3">
          ...
          {loadingSignups ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {subscriptions.map((s) => (
                <div key={s.id} className="bg-white border border-slate-200 p-3 rounded-lg text-xs font-mono text-slate-800 card-shadow">
                  {s.email}
                </div>
              ))}
            </div>
