...
Player 23,Loriella Park,Shorts,2026-08-27 1800,57,3,4,3,4
...
export const LeagueOperationsDemo: React.FC = () => {
  ...
  const [csvScoreSource, setCsvScoreSource] = useState<string>("");
  ...
  const updateHole = (cardId: string, hole: number) => {
    setCards((current) =>
      current.map((card) =>
        card.id === cardId
          ? { ...card, hole: Math.max(1, Math.min(18, hole || 1)) }
          : card,
      ),
    );
    setPublished(false);
  ...
  const calculateTags = () => {
    ...
    const availableTags = taggedPlayers
      .map((result) => result.currentTag as number)
      .sort((a, b) => a - b);

    const finishOrder = [...taggedPlayers].sort(
      (a, b) =>
        a.score - b.score ||
        (a.currentTag as number) - (b.currentTag as number),
    );
        ...
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="mt-1 leading-relaxed">
              These workflows demonstrate how repetitive league-night coordination could be handled by deterministic rules with an organizer review step. Card-building and bag-tag rules remain placeholders to confirm with club leadership before implementation.
            </p>
          </div>
        </div>
      </div>
      ...
      <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 card-shadow">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-teal-700">
              ...
              UDisc CSV → tags entered → deterministic reassignment
            </div>
            <h2 className="mt-1 text-lg font-extrabold text-slate-900">Bag Tag Assignment</h2>
            <p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-600">
              UDisc remains the score system of record. Import the completed-round CSV, enter the physical tags that were put into the pool, review the finish order, then calculate the outgoing tags.
            </p>
          </div>
          <button
            onClick={resetTags}
            className="flex items-center gap-1.5 rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
          >
            <RotateCcw className="h-3.5 w-3.5" /> Reset import
          </button>
        </div>

        <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_auto] lg:items-center">
          <label className="flex cursor-pointer items-center gap-3 rounded-xl border-2 border-dashed border-teal-200 bg-teal-50 px-4 py-4 text-teal-900 hover:border-teal-300">
            <FileUp className="h-6 w-6 shrink-0" />
            <span className="min-w-0">
              <span className="block text-sm font-extrabold">Import UDisc CSV</span>
              <span className="block text-[11px] text-teal-800">Reads player name plus +/- or total score; no scores are re-entered here.</span>
            </span>
            <input
              type="file"
              accept=".csv,text/csv"
          ...
          <button
            onClick={() => loadCsvText(DEMO_UDISC_CSV, "udisc-demo-round.csv")}
            className="rounded-lg bg-slate-900 px-4 py-2.5 text-xs font-bold text-white hover:bg-slate-800"
          >
            Load demo UDisc CSV
          </button>
        </div>
        ...
        {tagResults.length === 0 ? (
          <div className="rounded-xl border border-dashed border-slate-300 p-7 text-center text-xs text-slate-500">
            Export the completed league round from UDisc, then drop the CSV here. For this leadership demo, use “Load demo UDisc CSV.”
          </div>
        ) : (
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <div className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200">
                <p className="truncate text-sm font-extrabold text-slate-900">{csvFileName}</p>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Imported file</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200">
                <p className="text-2xl font-extrabold text-slate-900">{tagResults.length}</p>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">UDisc players</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200">
                <p className="text-2xl font-extrabold text-slate-900">{tagPoolCount}</p>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Tags entered</p>
              </div>
              <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200">
                <p className="truncate text-sm font-extrabold text-slate-900">{csvScoreSource}</p>
                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Score field used</p>
              </div>
            </div>

            {duplicateTags.length > 0 && (
              <div className="rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs font-semibold text-rose-800">
                Duplicate tag number{duplicateTags.length > 1 ? "s" : ""}: {duplicateTags.map((tag) => `#${tag}`).join(", ")}. Fix the tag pool before assigning.
              </div>
            )}
              ...
              <table className="w-full min-w-[620px] text-left text-xs">
                <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500">
                  <tr>
                    ...
                    <th className="px-3 py-2">UDisc player</th>
                    <th className="px-3 py-2">UDisc score</th>
                    <th className="px-3 py-2">Tag put in</th>
                    <th className="px-3 py-2">Tag out</th>
                  </tr>
                </thead>
                <tbody>
                  {tagResults.map((result, index) => (
                    <tr key={result.playerId} className="border-t border-slate-100">
                      <td className="px-3 py-2 font-extrabold text-slate-500">{index + 1}</td>
                      <td className="px-3 py-2 font-bold text-slate-900">{result.name}</td>
                      <td className="px-3 py-2 font-bold text-slate-700">{result.score > 0 && csvScoreSource.includes("+/-") ? "+" : ""}{result.score}</td>
                      <td className="px-3 py-2">
                        <input
                          type="number"
                          min={1}
                          value={result.currentTag ?? ""}
                          placeholder="Tag #"
                          onChange={(event) => updateCurrentTag(result.playerId, event.target.value)}
                          className="w-24 rounded-md border border-slate-300 bg-white px-2 py-1 font-bold text-slate-900"
                        />
                      </td>
                      <td className="px-3 py-2">
                        {result.newTag ? (
                          <span className={`inline-flex rounded-full px-2 py-1 font-extrabold ${
                            result.currentTag && result.newTag < result.currentTag
            ...
            <div className="flex flex-col gap-3 rounded-xl border border-teal-200 bg-teal-50 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <p className="text-xs font-bold text-teal-900">Demo bag-tag rule</p>
                <p className="mt-0.5 text-[11px] text-teal-800">
                  Only players with a tag number entered participate in the tag pool. Lowest UDisc score gets the lowest tag in the pool; ties currently break toward the lower incoming tag. Confirm the actual club tie rule before implementation.
                </p>
              </div>
              <button
                onClick={calculateTags}
                disabled={tagPoolCount === 0 || duplicateTags.length > 0}
                className="flex items-center justify-center gap-1.5 rounded-lg bg-teal-700 px-4 py-2 text-xs font-bold text-white hover:bg-teal-600 disabled:cursor-not-allowed disabled:opacity-40"
              >
                <Trophy className="h-3.5 w-3.5" /> Assign {tagPoolCount || ""} Tags
              </button>
            </div>
          </div>
        )}
