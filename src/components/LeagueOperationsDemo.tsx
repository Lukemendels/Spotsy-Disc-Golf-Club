...
function balancedChunks(players: LeagueCheckIn[], target = 4): LeagueCheckIn[][] {
  if (players.length <= 5) return [players];
  const minCards = Math.ceil(players.length / 5);
  const maxCards = Math.max(minCards, Math.floor(players.length / 3));
  const ideal = Math.max(minCards, Math.min(maxCards, Math.round(players.length / target)));
  const base = Math.floor(players.length / ideal);
  const remainder = players.length % ideal;
  const chunks: LeagueCheckIn[][] = [];
  let offset = 0;
  for (let index = 0; index < ideal; index += 1) {
    const size = base + (index < remainder ? 1 : 0);
    chunks.push(players.slice(offset, offset + size));
    offset += size;
  }
  return chunks;
}
...
function buildCards(players: LeagueCheckIn[], targetCardSize: number, firstHole: number): DemoCard[] {
  const grouped = new Map<string, LeagueCheckIn[]>();
  [...players]
    .sort((a, b) => a.division.localeCompare(b.division) || (a.spotsyTag ?? 9999) - (b.spotsyTag ?? 9999) || a.name.localeCompare(b.name))
    .forEach((player) => grouped.set(player.division, [...(grouped.get(player.division) ?? []), player]));

  const cards: DemoCard[] = [];
  [...grouped.entries()].sort(([a], [b]) => a.localeCompare(b)).forEach(([division, divisionPlayers]) => {
    balancedChunks(divisionPlayers, targetCardSize).forEach((chunk) => {
...
export const LeagueOperationsDemo: React.FC = () => {
  ...
  const loadDemoRoster = () => { const demo = buildDemoCheckIns(); persistPlayers(demo); setCheckInClosed(false); };
  ...
  const movePlayer = (playerId: string, destinationCardId: string) => {
    setCards((current) => {
      ...
      return without.map((card) => card.id === destinationCardId ? { ...card, players: [...card.players, moving as LeagueCheckIn] } : card);
    });
    setPublished(false);
  };
  ...
  const calculateTags = () => {
    ...
    const withSpotsy = assignTagPool(tagResults, "spotsyTag", "spotsyOut");
    setTagResults(assignTagPool(withSpotsy, "staffordTag", "staffordOut"));
  };
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 card-shadow">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-green-700"><QrCode className="h-4 w-4" />Before 5:45 PM · player QR check-in</div>
            <h2 className="mt-1 text-lg font-extrabold text-slate-900">League Check-In</h2>
            <p className="mt-1 max-w-2xl text-xs leading-relaxed text-slate-600">Players scan at the check-in table and submit their name, division, ace-pot status, Spotsy tag and/or Stafford tag. The roster locks at 5:45 for card generation.</p>
          </div>
          <div className="rounded-xl border border-slate-200 bg-white p-3 text-center shadow-sm">
            <QRCodeSVG value={checkInUrl} size={148} level="M" marginSize={2} />
            <a href={checkInUrl} target="_blank" rel="noreferrer" className="mt-2 block text-[10px] font-bold text-green-700 hover:underline">Open player check-in preview</a>
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3 sm:grid-cols-5">
          <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-2xl font-extrabold text-slate-900">{players.length}</p><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Checked in</p></div>
          <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-2xl font-extrabold text-amber-700">{acePotCount}</p><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Ace pot paid</p></div>
          <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-2xl font-extrabold text-green-700">{spotsyTagCount}</p><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Spotsy tags</p></div>
          <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-2xl font-extrabold text-blue-700">{staffordTagCount}</p><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Stafford tags</p></div>
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={loadDemoRoster} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50">Load 28 demo check-ins</button>
      ...
      <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 card-shadow">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-green-700"><Shuffle className="h-4 w-4" />5:45 lock → division cards → starting holes</div><h2 className="mt-1 text-lg font-extrabold text-slate-900">Card Builder</h2><p className="mt-1 max-w-2xl text-xs text-slate-600">Cards are built within division first, balanced toward four players, assigned sequential starting holes, then left open for organizer exceptions.</p></div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-2xl font-extrabold">{players.length}</p><p className="text-[10px] font-bold uppercase text-slate-500">Roster</p></div>
          <label className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><span className="block text-[10px] font-bold uppercase text-slate-500">Target card size</span><select value={targetCardSize} onChange={(event) => { setTargetCardSize(Number(event.target.value)); setCards([]); }} className="mt-1 w-full bg-transparent text-lg font-extrabold outline-none"><option value={3}>3</option><option value={4}>4</option><option value={5}>5</option></select></label>
          ...
          <div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-2xl font-extrabold">{cards.length || "—"}</p><p className="text-[10px] font-bold uppercase text-slate-500">Cards</p></div>
        </div>

        {cards.length > 0 && <div className="space-y-3"><div className="grid grid-cols-1 gap-3 lg:grid-cols-2">{cards.map((card, cardIndex) => <div key={card.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold uppercase text-slate-500">Card {cardIndex + 1} · {card.division}</p><p className="text-sm font-extrabold">{card.players.length} players</p></div><label className="text-[10px] font-bold uppercase text-slate-500">Hole <input type="number" min={1} max={18} value={card.hole} onChange={(event) => updateHole(card.id, Number(event.target.value) || 1)} className="ml-1 w-14 rounded border border-slate-300 bg-white px-2 py-1 text-center text-sm font-extrabold text-slate-900" /></label></div><div className="space-y-2">{card.players.map((player) => <div key={player.id} className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2"><div><p className="text-xs font-bold">{player.name}</p><p className="text-[10px] text-slate-500">{player.division} · Spotsy {player.spotsyTag ?? "—"} · Stafford {player.staffordTag ?? "—"} · Ace {player.acePotPaid ? "✓" : "—"}</p></div><select value={card.id} onChange={(event) => movePlayer(player.id, event.target.value)} className="rounded border border-slate-300 bg-white px-1.5 py-1 text-[10px] font-bold">{cards.map((destination, destinationIndex) => <option key={destination.id} value={destination.id}>Card {destinationIndex + 1}</option>)}</select></div>)}</div></div>)}</div><div className="flex justify-end"><button onClick={() => setPublished(true)} className="flex items-center gap-1.5 rounded-lg bg-slate-900 px-4 py-2 text-xs font-bold text-white"><CheckCircle2 className="h-3.5 w-3.5" />Finalize for 6:00</button></div></div>}

        {published && cards.length > 0 && <div className="rounded-xl border border-green-300 bg-green-50 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2 text-xs font-bold uppercase text-green-800"><Clock3 className="h-4 w-4" />6:00 PM shotgun call-out order</div><p className="mt-1 text-[11px] text-green-800">Read in this order, then send each card to its starting hole.</p></div><button onClick={copyCallout} className="rounded-lg bg-white px-3 py-2 text-xs font-bold text-green-800 ring-1 ring-green-300"><ClipboardCopy className="mr-1 inline h-3.5 w-3.5" />{copied ? "Copied" : "Copy call-out"}</button></div><ol className="mt-3 space-y-2">{cards.map((card, index) => <li key={card.id} className="rounded-lg bg-white px-3 py-2 text-xs text-slate-800"><strong>{index + 1}. Hole {card.hole} — {card.division}:</strong> {card.players.map((player) => player.name).join(", ")}</li>)}</ol></div>}
      </section>

      <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 card-shadow">
        <div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-teal-700"><Tags className="h-4 w-4" />After round · UDisc → two tag pools</div><h2 className="mt-1 text-lg font-extrabold text-slate-900">Bag Tag Assignment</h2><p className="mt-1 max-w-2xl text-xs text-slate-600">UDisc remains the score system of record. Imported player names are matched to the check-in roster, so Spotsy and Stafford tag numbers are already known. Each league’s tags are reassigned independently from the same finish order.</p></div>
        ...
        {tagResults.length > 0 && <div className="space-y-3"><div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><div className="rounded-xl bg-slate-50 p-3 ring-1 ring-slate-200"><p className="truncate text-sm font-extrabold">{csvFileName}</p><p className="text-[10px] font-bold uppercase text-slate-500">File</p></div><div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-2xl font-extrabold">{tagResults.length}</p><p className="text-[10px] font-bold uppercase text-slate-500">Players</p></div><div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-sm font-extrabold">{csvScoreSource}</p><p className="text-[10px] font-bold uppercase text-slate-500">Score field</p></div><div className="rounded-xl bg-slate-50 p-3 text-center ring-1 ring-slate-200"><p className="text-sm font-extrabold">2 pools</p><p className="text-[10px] font-bold uppercase text-slate-500">Spotsy + Stafford</p></div></div><div className="overflow-x-auto rounded-xl border border-slate-200"><table className="w-full min-w-[760px] text-left text-xs"><thead className="bg-slate-50 text-[10px] font-bold uppercase text-slate-500"><tr><th className="px-3 py-2">Finish</th><th className="px-3 py-2">Player</th><th className="px-3 py-2">Score</th><th className="px-3 py-2">Spotsy in</th><th className="px-3 py-2">Spotsy out</th><th className="px-3 py-2">Stafford in</th><th className="px-3 py-2">Stafford out</th></tr></thead><tbody>{tagResults.map((result, index) => <tr key={result.playerId} className="border-t border-slate-100"><td className="px-3 py-2 font-extrabold text-slate-500">{index + 1}</td><td className="px-3 py-2 font-bold">{result.name}</td><td className="px-3 py-2">{result.score}</td><td className="px-3 py-2">{result.spotsyTag ?? "—"}</td><td className="px-3 py-2 font-extrabold text-green-700">{result.spotsyOut ?? "—"}</td><td className="px-3 py-2">{result.staffordTag ?? "—"}</td><td className="px-3 py-2 font-extrabold text-blue-700">{result.staffordOut ?? "—"}</td></tr>)}</tbody></table></div><div className="flex flex-col gap-2 rounded-xl border border-teal-200 bg-teal-50 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="text-xs font-bold text-teal-900">Demo rule pending club confirmation</p><p className="text-[11px] text-teal-800">For each league separately, lowest score receives the lowest tag in that league’s submitted pool; ties currently break by incoming tag.</p></div><button disabled={spotsyDuplicates.length > 0 || staffordDuplicates.length > 0} onClick={calculateTags} className="rounded-lg bg-teal-700 px-4 py-2 text-xs font-bold text-white disabled:opacity-40"><Trophy className="mr-1 inline h-3.5 w-3.5" />Assign both tag pools</button></div></div>}
        {tagResults.length === 0 && <div className="rounded-xl border border-dashed border-slate-300 p-7 text-center text-xs text-slate-500">Import the completed UDisc CSV after the round. The check-in roster supplies the tags that were put into play.</div>}
      </section>
    </div>
  );
};
